...
export async function findEq(name, col, val) {
  const r = await sb(`${name}?${col}=eq.${encodeURIComponent(val)}&select=*`);
  if (r.configured) {
    const rows = Array.isArray(r.rows) ? r.rows : [];
    return rows[0] || null;
  }
  return (mem[name] || []).find((x) => String(x[col]) === String(val)) || null;
}
