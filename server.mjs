...
import { loadBoard, loadHeadlines, loadWorld, searchAll } from "./sports.mjs";
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/world") {
      ...
      return json(res, await loadWorld(body.date));
    }
    if (req.method === "POST" && url.pathname === "/api/board") {
      const body = JSON.parse((await readBody(req)) || "{}");
      return json(res, await loadBoard(body.leagueId, body.date));
    }
    if (req.method === "POST" && url.pathname === "/api/headlines") {
      return json(res, await loadHeadlines());
    }
    if (req.method === "POST" && url.pathname === "/api/search") {
      const body = JSON.parse((await readBody(req)) || "{}");
      return json(res, await searchAll(body.q || ""));
    }
